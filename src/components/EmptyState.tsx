  ...
  return (
    <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-2xl border border-white/40 dark:border-slate-700/40 p-8 shadow-[var(--shadow-card)] text-center">
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="mt-4 px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-violet-500 text-white text-xs font-bold uppercase tracking-widest shadow-lg shadow-rose-500/30 active:scale-95 transition-transform"
        >
          {actionLabel}
        </button>
      )}
